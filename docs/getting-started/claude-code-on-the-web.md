...
Configure automatic dependency installation using [SessionStart hooks](../reference/hooks.md#sessionstart). This can be configured in your repository's `.claude/settings.json` file:
...
SessionStart hooks can persist environment variables for subsequent bash commands by writing to the file specified in the `CLAUDE_ENV_FILE` environment variable. For details, see [SessionStart hooks](../reference/hooks.md#sessionstart) in the hooks reference.
...
1. **Use Claude Code hooks**: Configure [sessionStart hooks](../reference/hooks.md#sessionstart) to automate environment setup and dependency installation.
...
* [Hooks configuration](../reference/hooks.md)
