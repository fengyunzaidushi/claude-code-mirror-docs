...
Customize sandbox behavior through your `settings.json` file. See [Settings](../configuration/settings.md#sandbox-settings) for complete configuration reference.
...
<Note>
  ...
  You can disable this escape hatch by setting `"allowUnsandboxedCommands": false` in your [sandbox settings](../configuration/settings.md#sandbox-settings). When disabled, the `dangerouslyDisableSandbox` parameter is completely ignored and all commands must run sandboxed or be explicitly listed in `excludedCommands`.
</Note>
...
* Cannot read files that are denied in your [Claude permission settings](../administration/iam.md#configuring-permissions)
...
* **IAM policies**: Combine with [permission settings](../administration/iam.md) for defense-in-depth
* **Development containers**: Use with [devcontainers](devcontainer.md) for additional isolation
* **Enterprise policies**: Enforce sandbox configurations through [managed settings](../configuration/settings.md#settings-precedence)
...
* [Security](../administration/security.md) - Comprehensive security features and best practices
* [IAM](../administration/iam.md) - Permission configuration and access control
* [Settings](../configuration/settings.md) - Complete configuration reference
* [CLI reference](../reference/cli-reference.md) - Command-line options including `-sb`
