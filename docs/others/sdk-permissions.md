...
4. **[Permission rules (settings.json)](../configuration/settings.md#permission-settings)** - Declarative allow/deny rules with integrated bash command parsing
...
Permission rules in `settings.json` provide declarative control with built-in bash command parsing. These rules are evaluated before `canUseTool` is called. For more details on settings configuration, see the [Claude Code settings documentation](../configuration/settings.md).
...
2. **Use permission rules** for static policies, especially bash commands (see [permission settings](../configuration/settings.md#permission-settings))
